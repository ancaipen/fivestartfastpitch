...
/**
 * Methods supporting a list of Games records.
 *
 * @since  1.0.0
 */
class GamesModel extends ListModel
{
	...
	*/
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		parent::populateState('t.tournament_name', 'ASC');
	...
	protected function getListQuery()
	{
		...
		{
			$query->where("ag.`age` = '".$db->escape($filter_age_id)."'");
		}
		...
		$orderCol  = $this->state->get('list.ordering', 'tournament_name');
		$orderDirn = $this->state->get('list.direction', 'ASC');
		
		if ($orderCol && $orderDirn)
		{
			$query->order($db->escape($orderCol . ' ' . $orderDirn));
			//$query->order('t.tournament_name '. $orderDirn, 'a.age_id '. $orderDirn, 'a.game_date '. $orderDirn, 'a.game_time '. $orderDirn);
		}
	...
	public function getItems()
	{
		...
		foreach ($items as $oneItem)
		{
			...
			$oneItem->game_type = !empty($oneItem->game_type) ? Text::_('COM_TS_GAMES_GAME_TYPE_OPTION_' . strtoupper(str_replace(' ', '_',$oneItem->game_type))) : '';
