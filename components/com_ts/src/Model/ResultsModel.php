...
class ResultsModel extends ListModel
{
	...
	public function GetScheduleResults($tournament_id, $age_id)
    {
        ...
        INNER JOIN #__ts_season s on s.season_id=t.season_id
        WHERE (t.is_deleted = 0 OR t.is_deleted IS NULL) AND g.game_active = 1 AND g.state <> -2 AND 
        ...
        $query .= " ORDER BY g.game_date asc, g.game_time asc, g.game_order, g.game_type desc ";
        ...
        foreach ($rows as $row)
        {
			...
			$game_full_date = $game_date . ' ' . $game_time;
            $home_score = '';
            $visitor_score = '';
            if($row->home_score != -1)
            {
			...
			//don't display score unless datetime is greater than today
			$game_now = time();
			if(strtotime($game_date) > $game_now) 
			{
				$home_score = '';
				$visitor_score = '';
			}												  
			...
			//check next row
			$game_date_next = strtotime($rows[$row_count+1]->game_date);
			$game_date_next = date("F j, Y",$game_date_next);
			
            if($game_date_next!=$curr_date)									   
            {
                $html = $html . '</table>';
            }
            $row_count++;
	...
	public function GetFieldText($tournament_id, $age_id)
    {
        ...
        foreach ($rows as $row)
		{
			...
			$html = $html . '<div class="tourn_info">'. $row->tournament_results . '</div>';
			$html = $html .'<div class="tourn_title">Field Locations:</div> ';
			$html = $html . '<div class="tourn_info">' . $row->field_location_description. '</div>';
		}
	...
	public  function GetPoolText($tournament_id, $age_id)
     {
		...
		AND TRIM(UPPER(g.game_type)) = 'POOL' AND UPPER(home_team) NOT LIKE '%SEED%' AND UPPER(visitor_team) NOT LIKE '%WINNER%' 																													  
        UNION ALL
        ...
        WHERE (t.is_deleted = 0 OR t.is_deleted IS NULL) AND (g.state <> -2) AND 
        t.season_id in (SELECT season_id FROM #__ts_season WHERE season_current = 1) AND g.tournament_id = ".($tournament_id)." and g.age_id = ".($age_id)."
		AND TRIM(UPPER(g.game_type)) = 'POOL' AND UPPER(visitor_team) NOT LIKE '%SEED%' AND UPPER(visitor_team) NOT LIKE '%WINNER%' 																														 
        ) as pools
        WHERE pool <> ''
        ...
        foreach ($rows as $row)
        {
            ...
            if ($pool != $row->pool)
            {
                ...
                $html = $html . '<tr><td with="30%;">&nbsp;</td>';
				$html = $html . '<td style="text-align: center; font-weight:bold; color: #002964;" width="35px;">Wins</td>';
				$html = $html . '<td style="text-align: center; font-weight: bold; color: #002964;" width="35px;">Losses</td>';
				$html = $html . '<td style="text-align: center; font-weight: bold; color: #002964;" width="35px;">Ties</td>';
				$html = $html . '<td style="text-align: center; font-weight: bold; color: #002964;" width="35px;">Runs Scored</td>';
				$html = $html . '<td style="text-align: center; font-weight: bold; color: #002964;" width="35px;">Runs Allowed</td>';
				$html = $html . '<td style="text-align: center; font-weight: bold; color: #002964;" width="35px;">Run Diff</td></tr>';
                $pool = $row->pool;
            }
            ...
            $html = $html . '<tr><td width="30%;" class="'.$css.'">' . $title_count. '. ' . $row->team_name . '</td>'; 
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'"> ' . $wins .'</td>';
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'">'.$losses.'</td>';
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'">'.$ties .'</td>';
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'">'.$runs_scored .'</td>';
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'">'.$runs .'</td>';
			$html = $html . '<td style="text-align: center; font-weight: bold;" width="35px;" class="'.$css.'">'.$runs_diff .'</td></tr>';
